export class TimelineAssessor {
    ...
    detectMunicipalNotice(matter, evidenceIndex) {
        const description = (matter.description || '').toLowerCase();
        const tags = (matter.tags || []).map((t) => (t || '').toLowerCase());
        const partyNames = (matter.parties?.names || []).map((n) => (n || '').toLowerCase());
        ...
        if (evidenceIndex && Array.isArray(evidenceIndex.items)) {
            foundInEvidence = evidenceIndex.items.some((it) => {
        ...
        if (required) {
            return {
                required: true,
                message: 'Municipal notice may be required (10-day notice for municipal property/tree damage). Consider sending a formal notice to the municipal clerk/owner within 10 days; verify with local by-laws.'
            };
        }
        return { required: false };
    }
}
//# sourceMappingURL=TimelineAssessor.js.map