...
export class StyleGuide {
    ...
    check(text) {
        const warnings = [];
        if (advisoryWords.test(text))
            warnings.push('Advisory language detected; rephrase to informational tone.');
        if (emotionalTone.test(text))
            warnings.push('Emotional tone detected; keep restrained and factual.');
        if (text.length > 0 && !/[\.\?\!]/.test(text))
            warnings.push('Consider concise sentences with clear punctuation.');
