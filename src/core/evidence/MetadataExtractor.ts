...
export function extractMetadata(type: EvidenceType, content: Buffer): EvidenceMetadata {
  ...
  if (type === 'EML') {
    ...
    if (dateMatch) meta.date = normalizeDate(dateMatch[1]);
    if (fromMatch) meta.sender = fromMatch[1].trim();
    if (toMatch) meta.recipient = toMatch[1].trim();
    ...
    if (isoMatch) meta.date = `${isoMatch[1]}${isoMatch[2] ? 'T' + isoMatch[2] : ''}`;
